...
action_date,user_id,
case
when first_date=action_date then 'new_user'
when priv_action=
lag(action_date) over (partition by user_id order by action_date) then 'repeat_user'
else 'comeback_user' end as c,
priv_action
from daily_user_action)

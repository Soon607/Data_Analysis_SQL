  ...
  ```sql
	  with
		...
		cross join mst_intervals as i
		  where index_month <= to_char(current_date, 'YYYY-MM')
		),
...
* 신청일과 숙박일의 리드 타임을 계산하는 쿼리
  ```sql
	...
	select
	reservation_id,register_date,visit_date,
...
* 사용자들의 등록일부터 경과한 일수별 매출 계산
  ```sql
	...
	select
		u.user_id,u.b.base_date,
		cast(a.stamp as date) as action_date,
		max(cast(a.stamp as date)) over() as latest_date,
		substring(u.base_date,1,7) as month,
		r.index_name,
		cast(u.base_date::date+'1 day'::interval*r.interval_begin_date as date) as index_begin_date,
		cast(u.base_date::date+'1 day'::interval*r.interval_end_date as date) as index_end_date,
		a.amount
	from mst_users_with_base_date as u
	left join action_log as a
	on u.user_id=a.user_id 
	and a.action='purchase'
	cross join index_intervals as r)
	
	select * from
	purchase_log_with_index_date
  ```
* 월별 등록자 수&경과일 수별 매출 집계
  ```sql
	  with
	index_intervals(index_name,interval_begin_date,interval_end_date) as(
	values
	...
	select
		...
		r.index_name,
		cast(u.base_date::date+'1 day'::interval*r.interval_begin_date as date) as index_begin_date,
		cast(u.base_date::date+'1 day'::interval*r.interval_end_date as date) as interval_end_date,
		a.amount
	from mst_users_with_base_date as u
	left join action_log as a
	on u.user_id=a.user_id 
	...
	select
		...
		index_name,
		sum(case
		when interval_end_date<=latest_date then
		when action_date between index_begin_date and index_end_date then amount else 0 end end) as index_date_amount
